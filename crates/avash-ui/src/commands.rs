...
mod tests {
    ...
    pub(super) fn with_ssh_config(contents: &str) -> HomeGuard {
        let lock = HOME_LOCK
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
...
// ---------- Import de sessions (PuTTY, MobaXterm) ----------
...
/// Écrit les hôtes retenus dans `~/.ssh/config`, dans l'ordre. Un alias déjà
/// pris entre-temps est renommé plutôt que refusé. Rend le nombre écrit.
#[tauri::command]
pub fn import_apply(hosts: Vec<SshHost>) -> Result<usize, String> {
...
#[cfg(test)]
mod tests_import {
    use super::tests::with_ssh_config;
    use super::{import_apply, import_scan};
    ...
    fn apply_ecrit_les_hotes_avec_leur_dossier_et_renomme_les_collisions() {
        ...
        assert_eq!(import_apply(hotes).unwrap(), 2);
