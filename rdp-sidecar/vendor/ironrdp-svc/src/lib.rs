...
/// Plafond de réassemblage d'un PDU de canal statique, en octets (16 Mio).
///
/// Trouvé par l'audit du 7 septembre 2026 : sans borne, `dechunkify` accumulait
/// les morceaux sans fin (voir le commentaire dans `dechunkify`). 16 Mio
/// couvrent largement le texte du presse-papiers, les listes de fichiers et le
/// son ; au-delà, c'est un abus.
const REASSEMBLAGE_MAX: usize = 16 * 1024 * 1024;
...
impl ChunkProcessor {
    ...
    fn dechunkify(&mut self, payload: &[u8]) -> DecodeResult<Option<Vec<u8>>> {
        ...
        let (last, total_length) = Self::process_header(&mut cursor)?;

        // Trouvé par l'audit du 7 septembre 2026 : `dechunkify` réassemblait les
        // morceaux d'un canal statique sans aucune borne (le champ `length` du
        // CHANNEL_PDU_HEADER, longueur totale du PDU, était décodé puis ignoré).
        // Un serveur hostile faisait alors grossir `chunked_pdu` sans fin — des
        // morceaux FIRST/intermédiaires sans jamais poser FLAG_LAST, ou une
        // réponse d'un gigaoctet — jusqu'à l'OOM du sidecar, sur N'IMPORTE quel
        // canal statique (cliprdr, rdpdr, rdpsnd, drdynvc). Le « plafond 8 Mio »
        // du presse-papiers (presse_papiers.rs) ne bornait, lui, que la
        // transmission au front, en aval du réassemblage complet et du décodage.
        // On refuse donc ici, AVANT d'allouer, un `length` annoncé au-delà du
        // plafond, puis un cumul qui le dépasserait ; 16 Mio couvrent largement
        // texte, listes de fichiers et son.
        if total_length as usize > REASSEMBLAGE_MAX {
            self.chunked_pdu = Vec::new();
            return Err(invalid_field_err!(
                "CHANNEL_PDU_HEADER",
                "length",
                "PDU de canal statique trop grand"
            ));
        }
        let reste = cursor.remaining();
        if self.chunked_pdu.len().saturating_add(reste.len()) > REASSEMBLAGE_MAX {
            self.chunked_pdu = Vec::new();
            return Err(invalid_field_err!(
                "CHANNEL_PDU_HEADER",
                "length",
                "réassemblage de canal statique au-delà du plafond"
            ));
        }

        // Extend the chunked_pdu buffer with the payload
        self.chunked_pdu.extend_from_slice(reste);
    ...
    /// Returns whether this was the last chunk (based on the flags) and the total
    /// length announced in the channel header (used to bound reassembly).
    fn process_header(payload: &mut ReadCursor<'_>) -> DecodeResult<(bool, u32)> {
        let channel_header: ironrdp_pdu::rdp::vc::ChannelPduHeader = decode_cursor(payload)?;

        Ok((
            channel_header.flags.contains(ChannelControlFlags::FLAG_LAST),
            channel_header.length,
        ))
    }
...
// Trouvé par l'audit du 7 septembre 2026 : `dechunkify` réassemblait sans borne
// les morceaux d'un canal statique, un serveur hostile pouvant faire grossir
// `chunked_pdu` jusqu'à l'OOM du sidecar. Ces tests éprouvent le plafond de
// réassemblage (voir `REASSEMBLAGE_MAX`). Le manifeste porte désormais
// `test = true` (retiré) pour qu'ils s'exécutent vraiment.
#[cfg(test)]
mod tests_reassemblage {
    use super::{ChannelFlags, ChunkProcessor, REASSEMBLAGE_MAX};

    /// Fabrique un morceau de canal statique : CHANNEL_PDU_HEADER (length puis
    /// flags, deux u32 petit-boutistes) suivi des données.
    fn morceau(length: u32, flags: ChannelFlags, donnees: &[u8]) -> Vec<u8> {
        let mut v = Vec::with_capacity(8 + donnees.len());
        v.extend_from_slice(&length.to_le_bytes());
        v.extend_from_slice(&flags.bits().to_le_bytes());
        v.extend_from_slice(donnees);
        v
    }

    /// Un flux de morceaux qui ne pose jamais FLAG_LAST était accumulé sans fin :
    /// il faut un rejet AVANT de dépasser le plafond, pas un OOM. Sans le
    /// correctif, cette boucle ne rendait jamais d'erreur (elle gonflait la
    /// mémoire) ; avec, elle rend une erreur au plus tard au franchissement des
    /// 16 Mio.
    #[test]
    fn un_flux_sans_dernier_morceau_est_refuse_avant_l_oom() {
        let mut cp = ChunkProcessor::new();
        // length annoncé à 0 (le champ n'oblige à rien) : c'est le cumul qui doit
        // borner. Des morceaux de 1 Mio, sans jamais FLAG_LAST.
        let bloc = vec![0u8; 1024 * 1024];
        let premier = morceau(0, ChannelFlags::FIRST, &bloc);
        assert!(cp.dechunkify(&premier).is_ok());
        let suite = morceau(0, ChannelFlags::empty(), &bloc);
        let mut refuse = false;
        // 32 tours au plus : le plafond est à 16 Mio, l'erreur tombe bien avant.
        for _ in 0..32 {
            if cp.dechunkify(&suite).is_err() {
                refuse = true;
                break;
            }
        }
        assert!(refuse, "le réassemblage sans FLAG_LAST aurait dû être refusé");
    }

    /// Un `length` gigantesque annoncé dans l'en-tête (une FormatDataResponse
    /// d'un gigaoctet) est refusé d'emblée, sans réassembler ni allouer.
    #[test]
    fn un_length_annonce_geant_est_refuse_sans_allouer() {
        let mut cp = ChunkProcessor::new();
        let enorme = morceau(1024 * 1024 * 1024, ChannelFlags::FIRST, &[0u8; 4]);
        assert!(cp.dechunkify(&enorme).is_err());
    }

    /// Un PDU complet de taille raisonnable (FIRST|LAST) passe inchangé : le
    /// plafond ne gêne pas le trafic normal du presse-papiers.
    #[test]
    fn un_pdu_complet_raisonnable_passe() {
        let mut cp = ChunkProcessor::new();
        let charge = b"bonjour";
        let pdu = morceau(charge.len() as u32, ChannelFlags::FIRST | ChannelFlags::LAST, charge);
        assert_eq!(cp.dechunkify(&pdu).unwrap().as_deref(), Some(&charge[..]));
    }

    /// Le plafond est bien à 16 Mio (garde-fou de non-régression de la valeur).
    #[test]
    fn le_plafond_de_reassemblage_est_de_seize_mio() {
        assert_eq!(REASSEMBLAGE_MAX, 16 * 1024 * 1024);
    }
}
