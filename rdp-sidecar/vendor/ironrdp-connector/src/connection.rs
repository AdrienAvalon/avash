...
pub struct ClientConnector {
    ...
    pub message_channel_id: Option<u16>,
    /// Début de la mesure de bande passante à la connexion, s'il y en a une en cours.
    bw_debut: Option<std::time::Instant>,
    /// Octets reçus depuis le début de cette mesure.
    bw_octets: u32,
}
...
/// Répond à la détection automatique des caractéristiques réseau.
///
/// La version amont ne répondait qu'au RTT, en supposant que « le serveur passe
/// à la licence qu'il reçoive ou non le reste ». Contre un vrai serveur du parc
/// de test, c'est faux : il envoie Bandwidth Measure Start, une charge de
/// 16 Ko, puis Stop — et **attend les résultats**. Sans réponse, la séquence
/// reste suspendue là pour toujours, sans un mot.
///
/// On mesure donc réellement : instant de départ au Start, cumul des octets à
/// chaque charge, et à l'arrêt on renvoie le délai et le volume.
fn respond_to_connect_time_autodetect(
    request: rdp::autodetect::AutoDetectRequest,
    message_channel_id: u16,
    user_channel_id: u16,
    output: &mut WriteBuf,
    bw_debut: &mut Option<std::time::Instant>,
    bw_octets: &mut u32,
    ...
    match request {
        ...
        AutoDetectRequest::BandwidthMeasureStart { .. } => {
            *bw_debut = Some(std::time::Instant::now());
            *bw_octets = 0;
            Ok(Written::Nothing)
        }
        AutoDetectRequest::BandwidthMeasurePayload { payload, .. } => {
            *bw_octets = bw_octets.saturating_add(u32::try_from(payload.len()).unwrap_or(u32::MAX));
            Ok(Written::Nothing)
        }
        AutoDetectRequest::BandwidthMeasureStop {
            sequence_number, payload, ..
        } => {
            // À la connexion, l'arrêt transporte lui-même une dernière charge :
            // l'oublier fausserait la mesure d'autant.
            if let Some(p) = payload {
                *bw_octets = bw_octets.saturating_add(u32::try_from(p.len()).unwrap_or(u32::MAX));
            }
            let time_delta_ms = bw_debut
                .take()
        ...
        // Le résultat de caractéristiques réseau est purement informatif.
...
#[cfg(test)]
mod tests_autodetect {
    use super::{respond_to_connect_time_autodetect, WriteBuf, Written};
    use ironrdp_pdu::rdp::autodetect::AutoDetectRequest;

    fn stop(seq: u16) -> AutoDetectRequest {
        AutoDetectRequest::BandwidthMeasureStop {
            sequence_number: seq,
            request_type: 0x002B,
            payload: None,
    ...
    #[test]
    fn le_debut_et_la_charge_ne_repondent_rien_mais_comptent() {
        let mut out = WriteBuf::new();
        let (mut debut, mut octets) = (None, 0u32);
        let r = respond_to_connect_time_autodetect(
            AutoDetectRequest::BandwidthMeasureStart { sequence_number: 1, request_type: 0x0014 },
            1006, 1007, &mut out, &mut debut, &mut octets,
        )
        .unwrap();
        assert!(matches!(r, Written::Nothing));
        assert!(debut.is_some(), "l'instant de départ doit être retenu");

        respond_to_connect_time_autodetect(
            AutoDetectRequest::BandwidthMeasurePayload { sequence_number: 1, payload: vec![0; 1500] },
            1006, 1007, &mut out, &mut debut, &mut octets,
        )
        .unwrap();
        respond_to_connect_time_autodetect(
            AutoDetectRequest::BandwidthMeasurePayload { sequence_number: 1, payload: vec![0; 500] },
            1006, 1007, &mut out, &mut debut, &mut octets,
        )
        .unwrap();
        assert_eq!(octets, 2000, "les charges doivent s'additionner");
    }

    #[test]
    fn l_arret_repond_et_remet_le_compteur_a_zero() {
        let mut out = WriteBuf::new();
        let (mut debut, mut octets) = (Some(std::time::Instant::now()), 4096u32);
        let r = respond_to_connect_time_autodetect(stop(7), 1006, 1007, &mut out, &mut debut, &mut octets).unwrap();
        // C'est CE message manquant qui laissait la connexion suspendue sans fin.
        assert!(!matches!(r, Written::Nothing), "l'arrêt doit produire une réponse");
        assert_eq!(octets, 0);
        assert!(debut.is_none());
    }

    #[test]
    fn un_arret_sans_debut_ne_panique_pas() {
        // Un serveur peut envoyer l'arrêt sans début : cela doit répondre, pas planter.
        let mut out = WriteBuf::new();
        let (mut debut, mut octets) = (None, 0u32);
        let r = respond_to_connect_time_autodetect(stop(1), 1006, 1007, &mut out, &mut debut, &mut octets).unwrap();
        assert!(!matches!(r, Written::Nothing));
    }

    #[test]
    fn la_charge_portee_par_l_arret_est_comptee() {
        let mut out = WriteBuf::new();
        let (mut debut, mut octets) = (Some(std::time::Instant::now()), 1000u32);
        let arret = AutoDetectRequest::BandwidthMeasureStop {
            sequence_number: 3,
            request_type: 0x002B,
            payload: Some(vec![0; 250]),
        };
        // On lit le compteur au moment de répondre : il doit inclure ces 250 octets.
