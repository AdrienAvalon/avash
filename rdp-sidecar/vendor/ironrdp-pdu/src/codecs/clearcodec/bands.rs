...
fn decode_vbar<'a>(src: &mut ReadCursor<'a>, band_height: u16) -> DecodeResult<VBar<'a>> {
    ...
    //
    // Les deux champs étaient lus dans l'ordre inverse. La conséquence n'était
    // pas un rendu approximatif mais un refus systématique : `yOn` sur huit bits
    // dépasse presque toujours `yOff` sur six, et le contrôle de cohérence en
    // dessous rejetait alors chaque image. Aucun bureau Windows ne s'affichait
    // par ce codec — celui-là même que Windows emploie pour l'essentiel de son
    // dessin sur le canal graphique.
...
mod tests {
    ...
    fn decode_vbar_short_cache_miss() {
        ...
        //
        // Ce test encodait le défaut : son attente était dérivée de la même
        // lecture inversée que le décodeur, si bien qu'il passait pendant que
        // ClearCodec refusait toute image venue d'un vrai serveur. Il compose
        // maintenant le mot comme le fait la spécification — yOn en bas, yOff
        // au-dessus — et échouerait si l'ordre revenait en arrière.
        let y_on: u16 = 2;
        let y_off: u16 = 5;
    ...
    #[test]
    fn short_vbar_cache_miss_lit_yon_en_bas_et_yoff_en_haut() {
        // MS-RDPEGFX 2.2.4.1.1.2.1.1.3 : shortVBarYOn occupe les huit bits de
        // poids faible, shortVBarYOff les six suivants. Les deux champs étaient
        // lus dans l'ordre inverse, ce qui rendait presque toujours
        // `yOff < yOn` — et faisait refuser chaque image envoyée par un serveur
        // Windows, seul utilisateur courant de ce codec.
        let yon = 4u16;
        let yoff = 20u16;
        let mot = (yoff << 8) | yon;
        let mut donnees = mot.to_le_bytes().to_vec();
        donnees.extend(std::iter::repeat_n(0u8, usize::from(yoff - yon) * 3));
        let mut curseur = ReadCursor::new(&donnees);
        match decode_vbar(&mut curseur, 64).expect("barre lisible") {
            VBar::ShortCacheMiss(m) => {
                assert_eq!(m.y_on, 4);
                assert_eq!(m.y_off_delta, 16, "20 - 4 lignes de couleur");
                assert_eq!(m.pixel_data.len(), 16 * 3);
            }
            autre => panic!("attendu un défaut de cache court, obtenu {autre:?}"),
        }
    }
}
