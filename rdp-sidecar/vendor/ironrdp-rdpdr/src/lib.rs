...
impl Rdpdr {
    ...
    fn handle_client_id_confirm(&mut self, req: VersionAndIdPdu) -> PduResult<Vec<SvcMessage>> {
        ...
        // Correctif porté (avash) : la liste part même vide. Windows 11
        // (version 1.13) attend une Client Device List Announce après sa
        // Server Client ID Confirm avant d'envoyer Server User Logged On ;
        // sans elle, aucun lecteur n'est jamais annoncé (le dialogue
        // s'arrêtait là, éprouvé contre un Windows 11 du parc). mstsc et
        // FreeRDP envoient toujours cette liste, à zéro périphérique s'il le
        // faut (MS-RDPEFS 2.2.2.9), et le serveur de test ne s'en formalise
        // pas.
...
mod tests_avash {
    use super::*;
    use crate::backend::noop::NoopRdpdrBackend;
    use crate::pdu::efs::{VersionAndIdPdu, VersionAndIdPduKind};

    fn compte(message: &SvcMessage) -> u32 {
        // Entête partagé (composant et identifiant de paquet, 4 octets), puis
        // DeviceCount sur 32 bits, petit-boutiste.
        let octets = message.encode_unframed_pdu().unwrap();
        u32::from_le_bytes([octets[4], octets[5], octets[6], octets[7]])
    }

    /// Correctif porté : à la confirmation d'un serveur récent (version 1.13,
    /// Windows 11), la liste des périphériques part même vide, sans quoi le
    /// serveur n'envoie jamais Server User Logged On et les lecteurs ne sont
    /// jamais annoncés. Vu contre un Windows 11 du parc, le 05/09/2026. Le
    /// lecteur, lui, attend toujours l'ouverture de session.
    #[test]
    fn la_confirmation_d_un_serveur_recent_fait_partir_une_liste_meme_vide() {
        let mut rdpdr = Rdpdr::new(Box::new(NoopRdpdrBackend), "poste".to_owned())
            .with_drives(Some(vec![(1, "Avash".to_owned())]));
        let confirmation = VersionAndIdPdu {
            version_major: 1,
            version_minor: 13,
            client_id: 12,
            kind: VersionAndIdPduKind::ServerClientIdConfirm,
        };
        let messages = rdpdr.handle_client_id_confirm(confirmation).unwrap();
        assert_eq!(messages.len(), 1, "une annonce part, à zéro périphérique");
        assert_eq!(compte(&messages[0]), 0);

        let apres = rdpdr.announce_post_logon_devices().unwrap();
        assert_eq!(apres.len(), 1, "le lecteur part à l'ouverture de session");
        assert_eq!(compte(&apres[0]), 1);
        assert!(rdpdr.announce_post_logon_devices().unwrap().is_empty(), "une seule fois");
    }

    /// Un serveur 5.1 (version mineure 5) reçoit tout d'un coup, comme avant.
    #[test]
    fn un_serveur_ancien_recoit_tout_a_la_confirmation() {
        let mut rdpdr = Rdpdr::new(Box::new(NoopRdpdrBackend), "poste".to_owned())
            .with_drives(Some(vec![(1, "Avash".to_owned())]));
        let confirmation = VersionAndIdPdu {
            version_major: 1,
        ...
        assert_eq!(compte(&messages[0]), 1);
        assert!(rdpdr.announce_post_logon_devices().unwrap().is_empty());
    }
}
