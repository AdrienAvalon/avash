...
/// Retire le remplissage de fin de ligne d'un bitmap décodé.
///
/// RDP autorise `bitmapWidth` à dépasser la largeur du rectangle de destination
/// (MS-RDPBCGR 2.2.9.1.1.3.1.2.2) : xrdp complète ainsi ses tuiles — 340 pixels
/// annoncés pour un rectangle de 337. Les fonctions `apply_*` relisent le
/// tampon en le découpant à la largeur du RECTANGLE ; si le tampon est écrit à
/// celle du BITMAP, chaque ligne glisse de la différence et l'image part en
/// biais. Le chemin non compressé retire déjà ce remplissage ; les chemins
/// compressés, non.
fn retirer_remplissage(buf: &mut Vec<u8>, largeur_bitmap: usize, largeur_rect: usize, hauteur: usize, octets_par_pixel: usize) {
    if largeur_bitmap <= largeur_rect {
        return;
    }
    let ligne = largeur_bitmap * octets_par_pixel;
    let garde = largeur_rect * octets_par_pixel;
    let mut ecrit = 0usize;
    for r in 0..hauteur {
        let debut = r * ligne;
        if debut + garde > buf.len() {
            break;
        }
        buf.copy_within(debut..debut + garde, ecrit);
        ecrit += garde;
    }
    buf.truncate(ecrit);
}
                    ...
                    match self.bitmap_stream_decoder.decode_bitmap_stream_to_rgb24(
                        ...
                        Ok(()) => {
                            retirer_remplissage(
                                &mut buf,
                                usize::from(update.width),
                                usize::from(update.rectangle.width()),
                                usize::from(update.height),
                                3,
                            );
...
#[cfg(test)]
mod tests_remplissage {
    use super::retirer_remplissage;

    #[test]
    fn un_bitmap_plus_large_que_le_rectangle_est_recadre() {
        // Deux lignes de 4 pixels annoncées, 3 seulement utiles (1 octet/pixel).
        let mut buf = vec![1, 2, 3, 99, 4, 5, 6, 99];
        retirer_remplissage(&mut buf, 4, 3, 2, 1);
        assert_eq!(buf, vec![1, 2, 3, 4, 5, 6], "le remplissage doit disparaître");
    }

    #[test]
    fn sans_remplissage_le_tampon_est_intact() {
        let avant = vec![1, 2, 3, 4, 5, 6];
        let mut buf = avant.clone();
        retirer_remplissage(&mut buf, 3, 3, 2, 1);
        assert_eq!(buf, avant);
    }

    #[test]
    fn plusieurs_octets_par_pixel_sont_respectes() {
        // 2 pixels annoncés, 1 utile, 3 octets par pixel, 2 lignes.
        let mut buf = vec![1, 2, 3, 9, 9, 9, 4, 5, 6, 9, 9, 9];
        retirer_remplissage(&mut buf, 2, 1, 2, 3);
        assert_eq!(buf, vec![1, 2, 3, 4, 5, 6]);
    }

    #[test]
    fn un_tampon_trop_court_ne_panique_pas() {
