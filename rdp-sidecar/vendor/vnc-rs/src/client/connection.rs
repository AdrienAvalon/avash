...
/// Plus grande résolution acceptée d'un serveur, dans chaque dimension. Au-delà,
/// le cadre ne tient dans aucune mémoire raisonnable (65535 × 65535 × 4 = 17 Gio)
/// et le client refuse la session plutôt que de la laisser mourir plus loin.
pub const TAILLE_MAX_CADRE: u16 = 8192;

fn resolution_sure(width: u16, height: u16) -> Result<(), VncError> {
    if width == 0 || height == 0 || width > TAILLE_MAX_CADRE || height > TAILLE_MAX_CADRE {
        return Err(VncError::General(format!(
            "le serveur annonce une résolution inacceptable ({width}x{height})"
        )));
    }
    Ok(())
}

/// Un rectangle de mise à jour doit tenir dans le cadre annoncé : les
/// décodeurs allouent à sa taille avant de lire, et le client le peint sans
/// autre garde.
fn rect_dans_le_cadre(rect: &Rect, screen: (u16, u16)) -> Result<(), VncError> {
    let tient = u32::from(rect.x) + u32::from(rect.width) <= u32::from(screen.0)
        && u32::from(rect.y) + u32::from(rect.height) <= u32::from(screen.1);
    if tient {
        Ok(())
    } else {
        Err(VncError::General(format!(
            "rectangle hors du cadre : {}x{} en ({}, {}) pour un cadre de {}x{}",
            rect.width, rect.height, rect.x, rect.y, screen.0, screen.1
        )))
    }
}
...
struct VncInner {
    ...
    /// Absent une fois pris par [`VncClient::take_events`].
    output_ch: Option<Receiver<VncEvent>>,
...
impl VncInner {
    ...
    async fn recv_event(&mut self) -> Result<VncEvent, VncError> {
        ...
        let Some(output_ch) = self.output_ch.as_mut() else {
            return Err(VncError::ClientNotRunning);
        };
        match output_ch.recv().await {
            Some(e) => Ok(e),
            None => {
                self.closed = true;
                Err(VncError::ClientNotRunning)
    ...
    fn take_events(&mut self) -> Option<Receiver<VncEvent>> {
        self.output_ch.take()
    }

    fn set_screen(&mut self, width: u16, height: u16) {
...
impl VncClient {
    ...
    /// Détache la file des événements du serveur.
    ///
    /// `recv_event` garde le verrou du client pendant toute son attente : une
    /// tâche qui y attend une image empêche toute autre d'envoyer une frappe
    /// ou un mouvement de souris par `input`, jusqu'à ce qu'un événement
    /// arrive. Avec la file en main, l'appelant attend les événements sans
    /// verrou et n'appelle `input` que pour écrire. Après cet appel,
    /// `recv_event` et `poll_event` rendent `ClientNotRunning` ; un second
    /// appel rend `None`.
    pub async fn take_events(&self) -> Option<Receiver<VncEvent>> {
        self.inner.lock().await.take_events()
    }

    /// Nouvelle taille du cadre, annoncée par le serveur (pseudo-codage
    /// DesktopSize). Les demandes de mise à jour (`X11Event::Refresh`)
    /// couvrent le cadre entier : sans cet appel elles gardaient la taille du
    /// début de session, et rien n'arrivait plus au-delà après un
    /// agrandissement.
    pub async fn set_screen(&self, width: u16, height: u16) {
        self.inner.lock().await.set_screen(width, height);
    }
}
        ...
        match server_msg {
            ServerMsg::FramebufferUpdate(rect_num) => {
                for _ in 0..rect_num {
                    ...
                    // Les pseudo-codages ne décrivent pas une zone du cadre
                    // (taille de bureau, curseur, fin de liste) ; tout le
                    // reste doit tenir dedans avant qu'un décodeur n'alloue.
                    let pseudo = matches!(
                        rect.encoding,
                    ...
                    if !pseudo {
                        rect_dans_le_cadre(&rect.rect, screen)?;
                    }
                    ...
                    match rect.encoding {
                        ...
                        VncEncoding::DesktopSizePseudo => {
                            resolution_sure(rect.rect.width, rect.rect.height)?;
