...
use super::{tampon, zlib::ZlibReader};
...
impl Decoder {
        ...
        Fut: Future<Output = Result<(), VncError>>,
    {
        ...
        self.palette = tampon(palette_size)?;
        ...
        Fut: Future<Output = Result<(), VncError>>,
    {
        ...
        let data = self
            .read_tight_data(stream, input, uncompressed_size)
            .await?;
        let mut image = tampon(rect.width as usize * rect.height as usize * 4)?;
