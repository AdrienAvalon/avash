...
use ironrdp_pdu::codecs::clearcodec::{
    decode_bands_layer, decode_residual_layer, decode_subcodec_layer, encode_residual_layer,
    ClearCodecBitmapStream, CompositePayload, RgbRunSegment, SubcodecId, VBar, FLAG_GLYPH_INDEX,
};
...
impl ClearCodecDecoder {
    ...
    fn decode_subcodec_region(
        ...
        match sub.codec_id {
            ...
            SubcodecId::NsCodec => {
                // Portage avash : upstream left this arm empty, and Windows
                // encodes the taskbar icons with it. See nscodec.rs.
                nscodec::decode(
                    sub.bitmap_data,
                    sub.width,
                    sub.height,
