        ...
        match descriptor {
            ...
            SegmentedDescriptor::Multipart => {
                ...
                for _ in 0..segment_count {
                    ...
                    // La taille vient du fil : `split_at` paniquait au-delà du
                    // message (audit du 12 septembre 2026, C-panique-1).
                    let (segment_data, new_buffer) = buffer.split_at_checked(size).ok_or_else(|| {
                        ZgfxError::IOError(std::io::Error::new(
                            std::io::ErrorKind::UnexpectedEof,
                            "segment ZGFX plus long que le message",
                        ))
                    })?;
                    buffer = new_buffer;
