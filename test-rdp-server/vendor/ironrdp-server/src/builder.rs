...
use super::server::{
    ConnectionHandler, CredentialValidator, RdpServer, RdpServerOptions, RdpServerSecurity, SvcServerFactory,
};
...
impl RdpServerBuilder<BuilderDone> {
    ...
    /// Attache un canal statique de plus, construit neuf à chaque connexion.
    ///
    /// Ajout d'avash (voir `vendor/README.md`) : un seul canal par ce chemin,
    /// voir [`SvcServerFactory`].
    pub fn with_static_channel_factory(mut self, factory: Option<Box<dyn SvcServerFactory>>) -> Self {
        self.state.svc_factory = factory;
        self
    }
