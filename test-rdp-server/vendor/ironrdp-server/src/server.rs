...
use ironrdp_svc::{
    ChannelFlags, CompressionCondition, StaticChannelId, StaticChannelSet, SvcMessage, SvcProcessor,
    SvcServerProcessor, server_encode_svc_messages,
};
...
pub struct RdpServer {
    ...
    /// Ajout d'avash : un canal statique de plus, servi par sa fabrique.
    svc_factory: Option<Box<dyn SvcServerFactory>>,
...
/// Fabrique d'un canal statique supplémentaire, hors cliprdr et rdpsnd que le
/// serveur attache lui-même. Il en demande un exemplaire neuf à chaque
/// connexion, comme il le fait pour le presse-papiers et le son : un canal
/// porte l'état d'une session, il ne survit pas au client.
///
/// Ajout d'avash (voir `vendor/README.md`) : l'amont n'offre aucun point
/// d'attache pour un canal statique arbitraire, ce dont le serveur de test a
/// besoin pour jouer le côté serveur de la redirection de lecteur (rdpdr).
pub trait SvcServerFactory: Send + Sync {
    fn build_svc(&self) -> Box<dyn SvcServerProcessor>;
}

/// Enveloppe qui rend attachable un canal en boîte.
///
/// `Acceptor::attach_static_channel` exige un type concret, dont il tire le
/// `TypeId` qui indexe le canal dans `StaticChannelSet` ; une boîte de trait
/// n'en a pas d'utile. Conséquence assumée : un seul canal peut passer par ce
/// chemin, une seconde fabrique remplacerait la première sous le même
/// `TypeId`. D'où une option, pas une liste.
#[derive(Debug)]
struct BoxedSvc(Box<dyn SvcServerProcessor>);

impl_as_any!(BoxedSvc);

impl SvcProcessor for BoxedSvc {
...
impl SvcServerProcessor for BoxedSvc {}
...
impl RdpServer {
    ...
    fn attach_channels(&mut self, acceptor: &mut Acceptor) {
        ...
        // Ajout d'avash : le canal n'est joint que si le client le demande par
        // son nom dans les données GCC, l'acceptor fait l'appariement ; sans
        // demande, la fabrique a tourné pour rien et le canal reste inerte.
        if let Some(factory) = self.svc_factory.as_deref() {
            acceptor.attach_static_channel(BoxedSvc(factory.build_svc()));
        }
