...
[![Tests](https://img.shields.io/badge/tests-1948%20verts-brightgreen.svg)](docs/qualite.md)
...
| **VNC** | les bureaux VNC dans la même fenêtre (ZRLE, clavier en keysyms, presse-papiers), chiffrés par VeNCrypt quand le serveur l'offre, certificat épinglé au premier contact |
...
signés pour la mise à jour automatique et accompagnés de leurs empreintes. Dans
les commandes ci-dessous, `x.y.z` est le numéro de la version téléchargée.
...
**1948 tests** à chaque commit, sur deux chaînes indépendantes (GitHub Actions
sur Linux, Windows et macOS ; un miroir GitLab avec de vrais serveurs xrdp) :

| Niveau | Tests | En un mot |
|---|---:|---|
| Cœur Rust et intégration contre un vrai sshd | 343 | parseurs, import, SFTP, tunnels, rebonds |
| Interface Tauri | 201 | commandes, magasin de sessions, clavier |
| Processus RDP | 237 | négociation, canal graphique, session VNC, fichiers par le presse-papiers, rejeu d'enregistrements réels, fuzzing par mutation |
...
| Paquets IronRDP et vnc-rs portés | 634 | nos correctifs, un serveur VNC hostile, et les tests amont qui ne s'exécutaient nulle part |
| Front (Vitest) | 420 | logique pure, keysyms VNC, traductions |
| Bout en bout (WebdriverIO) | 81 | l'application réelle, connexions SSH, RDP et VNC effectives, audit `axe-core` |
...
neuf cibles cargo-fuzz et un parc RDP en conteneurs. Une règle tient lieu de
...
l'interface par WebSocket binaire local. Cinq paquets IronRDP et le client
...
avash est distribué sous licence **[AGPL-3.0-or-later](LICENSE)**. Cette licence
autorise l'usage commercial et la vente de copies, à condition d'en partager les
sources dans les mêmes termes, y compris pour une mise à disposition comme
service en réseau. Elle ne crée aucune redevance et n'interdit ni la revente ni
les forks.

Pour un usage qui demande d'autres permissions, par exemple intégrer avash dans
un produit propriétaire sans l'obligation de partage de l'AGPL, une licence
commerciale distincte peut être négociée avec l'auteur :
[avalon-network.com/contact](https://avalon-network.com/contact). Elle s'ajoute
à l'AGPL sans retirer les droits déjà accordés aux versions publiées.

Les composants tiers gardent leurs propres licences et attributions : les
paquets IronRDP et vnc-rs portés (MIT, Apache-2.0) sont documentés dans
[rdp-sidecar/vendor/README.md](rdp-sidecar/vendor/README.md), et la police Meslo
Nerd Font dans
