...
[![Tests](https://img.shields.io/badge/tests-1233%20verts-brightgreen.svg)](docs/qualite.md)
...
```bash
chmod +x Avash_0.9.1_amd64.AppImage
./Avash_0.9.1_amd64.AppImage
```
...
```bash
sudo apt install ./Avash_0.9.1_amd64.deb      # Debian, Ubuntu
sudo dnf install ./Avash-0.9.1-1.x86_64.rpm    # Fedora
```
...
gh attestation verify Avash_0.9.1_amd64.AppImage --repo AdrienAvalon/avash   # provenance
```
...
**1233 tests** à chaque commit, sur deux chaînes indépendantes (GitHub Actions
sur Linux, Windows et macOS ; un miroir GitLab avec de vrais serveurs xrdp) :

| Niveau | Tests | En un mot |
|---|---:|---|
| Cœur Rust et intégration contre un vrai sshd | 205 | parseurs, import, SFTP, tunnels, rebonds |
| Interface Tauri | 72 | commandes, magasin de sessions, clavier |
| Processus RDP | 135 | négociation, canal graphique, session VNC, fichiers par le presse-papiers, rejeu d'enregistrements réels, fuzzing par mutation |
...
| Paquets IronRDP et vnc-rs portés | 597 | nos correctifs, un serveur VNC hostile, et les tests amont qui ne s'exécutaient nulle part |
| Front (Vitest) | 124 | logique pure, keysyms VNC, traductions |
| Bout en bout (WebdriverIO) | 71 | l'application réelle, connexions SSH, RDP et VNC effectives, audit `axe-core` |
